...
import * as ServerConfig from '@qwen/qwen-cli-core';
...
vi.mock('@qwen/qwen-cli-core', async () => {
...
describe('loadCliConfig', () => {
  ...
  beforeEach(() => {
    ...
    process.env.DASHSCOPE_API_KEY = 'test-api-key'; // Ensure API key is set for tests
  });
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
  ...
  it('should correctly use mocked homedir for global path', async () => {
    const MOCK_QWEN_DIR_LOCAL = path.join('/mock/home/user', '.qwen');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_QWEN_DIR_LOCAL, 'QWEN.md');
    mockFs({
