...
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setQwenMdFilename.
let currentQwenMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setQwenMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentQwenMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentQwenMdFilename = newFilename.trim();
  }
}

export function getCurrentQwenMdFilename(): string {
  if (Array.isArray(currentQwenMdFilename)) {
    return currentQwenMdFilename[0];
  }
  return currentQwenMdFilename;
}

export function getAllQwenMdFilenames(): string[] {
  if (Array.isArray(currentQwenMdFilename)) {
    return currentQwenMdFilename;
  }
  return [currentQwenMdFilename];
...
export const setGeminiMdFilename = setQwenMdFilename;
export const getCurrentGeminiMdFilename = getCurrentQwenMdFilename;
export const getAllGeminiMdFilenames = getAllQwenMdFilenames;
...
function getGlobalMemoryFilePath(): string {
  return path.join(homedir(), QWEN_CONFIG_DIR, getCurrentQwenMdFilename());
}
