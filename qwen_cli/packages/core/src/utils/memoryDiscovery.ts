...
async function getGeminiMdFilePathsInternal(
  ...
  const qwenMdFilenames = getAllQwenMdFilenames();

  for (const qwenMdFilename of qwenMdFilenames) {
    ...
    if (debugMode)
      logger.debug(
        `Searching for ${qwenMdFilename} starting from CWD: ${resolvedCwd}`,
      );
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${qwenMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
      if (debugMode)
        logger.debug(
          `Global ${qwenMdFilename} not found or not readable: ${globalMemoryPath}`,
        );
    }
    ...
    while (currentDir && currentDir !== path.dirname(currentDir)) {
      ...
      if (debugMode) {
        logger.debug(
          `Checking for ${qwenMdFilename} in (upward scan): ${currentDir}`,
        );
      }

      // Skip the global .qwen directory itself during upward scan from CWD,
      // as global is handled separately and explicitly first.
      if (currentDir === path.join(resolvedHome, QWEN_CONFIG_DIR)) {
        if (debugMode) {
          logger.debug(
      ...
      const potentialPath = path.join(currentDir, qwenMdFilename);
      try {
        ...
        if (potentialPath !== globalMemoryPath) {
          ...
          if (debugMode) {
            logger.debug(
              `Found readable upward ${qwenMdFilename}: ${potentialPath}`,
            );
          }
        }
      } catch {
        if (debugMode) {
          logger.debug(
            `Upward ${qwenMdFilename} not found or not readable in: ${currentDir}`,
          );
        }
      }
    ...
    if (debugMode && downwardPaths.length > 0)
      logger.debug(
        `Found downward ${qwenMdFilename} files (sorted): ${JSON.stringify(
          downwardPaths,
        )}`,
      );
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllQwenMdFilenames()} paths to read: ${JSON.stringify(
        finalPaths,
      )}`,
    );
  return finalPaths;
  ...
  for (const filePath of filePaths) {
    try {
      ...
      if (!isTestEnv) {
        ...
        logger.warn(
          `Warning: Could not read ${getAllQwenMdFilenames()} file at ${filePath}. Error: ${message}`,
        );
      }
