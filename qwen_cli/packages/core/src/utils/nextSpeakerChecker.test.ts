...
import { QwenChat } from '../core/qwenChat.js';
...
const mockGoogleGenAIInstance = {
  ...
  // Add other methods of GoogleGenAI if they are directly used by QwenChat constructor or its methods
...
vi.mock('@google/genai', async () => {
  ...
  return {
    ...
    // If Models is instantiated directly in QwenChat, mock its constructor too
    // For now, assuming Models instance is obtained via getGenerativeModel
  };
});

describe('checkNextSpeaker', () => {
  let chatInstance: QwenChat;
  let mockQwenClient: QwenClient;
  let MockConfig: Mock;
  ...
  beforeEach(() => {
    ...
    mockQwenClient = new QwenClient(mockConfigInstance);
    ...
    // QwenChat will receive the mocked instances via the mocked GoogleGenAI constructor
    chatInstance = new QwenChat(
      mockConfigInstance,
  ...
  it('should return null if history is empty', async () => {
    ...
    expect(mockQwenClient.generateJson).not.toHaveBeenCalled();
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockQwenClient.generateJson as Mock).mockResolvedValue(mockApiResponse);
    ...
    expect(mockQwenClient.generateJson).toHaveBeenCalledTimes(1);
  });
  ...
  it('should return null if geminiClient.generateJson throws an error', async () => {
    ...
    (mockQwenClient.generateJson as Mock).mockRejectedValue(
      new Error('API Error'),
    );
  ...
  it('should return null if geminiClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    ...
    (mockQwenClient.generateJson as Mock).mockResolvedValue({
