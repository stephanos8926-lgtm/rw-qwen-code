...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as QwenChat);
  ...
  describe('run', () => {
    it('should yield content events for text parts', async () => {
      ...
      expect(events).toEqual([
        { type: QwenEventType.Content, value: 'Hello' },
        { type: QwenEventType.Content, value: ' world' },
      ]);
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerQwenToolCallRequestEvent;
      expect(event1.type).toBe(QwenEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerQwenToolCallRequestEvent;
      expect(event2.type).toBe(QwenEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: QwenEventType.Content, value: 'First part' },
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerQwenErrorEvent;
      expect(errorEvent.type).toBe(QwenEventType.Error);
    ...
    it('should yield the last UsageMetadata event from the stream', async () => {
      ...
      const metadataEvent = events[2] as ServerQwenUsageMetadataEvent;
      expect(metadataEvent.type).toBe(QwenEventType.UsageMetadata);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerQwenToolCallRequestEvent;
      expect(event3.type).toBe(QwenEventType.ToolCallRequest);
